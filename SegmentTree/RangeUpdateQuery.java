...
class RangeUpdateQuery
{
    ...
    public static void update(int idx,int ind,int val,int l,int h)
    {
        if(l==h)
        {
            seg[ind]+=val;
        ...
        int m=(l+h)/2;
        if(idx<=m)
        update(idx,2*ind+1,val,l,m);
        else
        update(idx,2*ind+2,val,m+1,h);
   ...
   public static int querying(int ind, int low, int high, int l, int h) {
     ...
     public static void main(String args[])
    {
        ...
        for(int i=0;i<q;i++)
        {
            ...
            update(0,query[i][0],query[i][1],0,5);
            System.out.println(querying(0,0, n-1,  0, n-1));
        }
